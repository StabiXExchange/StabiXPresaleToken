...
contract StabiXPresaleToken is ERC20, Ownable {
  ...
  string public constant name = "StabiX Presale Token";
  string public constant symbol = "SXPT";
...
/**
 * @title StabiXPresale 
 * StabiXPresale allows investors to make
 * token purchases and assigns them tokens based
 * on a token per ETH rate. Funds collected are forwarded to a wallet 
 * as they arrive.
 */
 
contract StabiXPresale is Pausable {
  using SafeMath for uint256;

  StabiXPresaleToken public token; 
  ...
  function StabiXPresale(address _wallet, uint256 _minInvestment, uint256 _cap, uint256 _rate) {
    ...
    wallet = _wallet;
    rate = _rate;
    minInvestment = _minInvestment;  //minimum investment in wei  (=0.01 ether)
    cap = _cap * (10**18);  //cap in tokens base units (=10,000,000 tokens)
  }

  // creates presale token
  function createTokenContract() internal returns (StabiXPresaleToken) {
    return new StabiXPresaleToken();
  }
