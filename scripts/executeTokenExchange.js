module.exports = function(callback) {
    ...
    const executeScript = async function(crowdsale, token) {
        ...
        console.log("Token distributino before token transfer ");
        await printTotalTokenSupply(token);
        await printTokenDistribution(token, investors);

        transactionInfo = await token.transfer(investors[1], 100, {from: investors[0], gas: 100000});

        //we wait until the transaction is mined
        transactionReceipt = await waitUntilTransactionsMined(transactionInfo.tx);
            
        console.log("Token distribution after transfer :");
        await printTotalTokenSupply(token);
        await printTokenDistribution(token, investors);
    ...
    const printInvestorSending = (amounts) => {
        let index = 1;
        amounts.forEach((amount) => {
            console.log(`Investor ${index} is sending ${amount} tokens`);
            index++;
        });
        skipLine();
    }

    const buildTokenTransfers = (crowdsale, investors, tokenAmounts) => {

        transactionObjects = [];

        for (i = 0; i < investors.length; i++) {
            txn = { from: investors[i], to: investors[i+1], gas: 1000000 }
