module.exports = function(callback) {
    ...
    StabiXPresaleTokenArtifact = require('../build/contracts/StabiXPresaleToken.json');
    ...
    const run = async function() {
        ...
        let presaleTokenContract = contract(StabiXPresaleTokenArtifact);
    ...
    const executeScript = async function(crowdsale, token) {
        ...
        let investorAmounts = ['100', '0.1', '1'].map((amount) => {return inWei(amount)});

        printInvestorBalances(investors);
        printInvestorAddresses(investors);

        await verifyCrowdsaleContactInformation(crowdsale, owner);
        await verifyCrowdsaleNotHalted(crowdsale);
        ...
        totalTokenSupply = await getTokenTotalSupply(token);
        console.log(`The total token supply created is ${totalTokenSupply}`);
        tokenDistribution = await getTokenDistribution(token, investors);
        printTokenDistribution(tokenDistribution);
        printInvestorBalances(investors);
    ...
    const printInvestorBalances = (addresses) => {
        ...
        addresses.forEach((address) => {
            ...
            console.log(`Investor ${index} wallet balance : ${currentBalance}`);
            index++;
        });
        skipLine();
    }
    ...
    const printInvestorBuying = (amounts) => {
        ...
        amounts.forEach((amount) => {
            console.log(`Investor ${index} is sending ${amount} (${inEther(amount)}) to the contract`);
            index++;
        });
        skipLine();
    }
    ...
    const verifyCrowdsaleContactInformation = async (crowdsale, owner) => {
        ...
        await crowdsale.setContactInformation('shlyakhovoy@stabix.exchange', {from: owner, gas: 1000000});
    ...
    // const waitUntilTransactionsMined = functions(txn_hashes) {

    //     const promises = txn_hashes.map(async (address) => {
    //         return { address: address, balance: await waitUntilTransactionMined(txn_hash) };
    //     });
